...
import { IconDashboard, IconList, IconPower } from '@tabler/icons';
...
export const SiderWrapper: React.FC<RefineThemedLayoutV2SiderProps> = ({
  ...
  const borderColor = theme.colorScheme === 'dark' ? theme.colors.dark[6] : theme.colors.gray[2];

  const commonNavLinkStyles: Styles<NavLinkStylesNames, NavLinkStylesParams> = {
    root: {
      display: 'flex',
      marginTop: '12px',
      justifyContent: siderCollapsed && !mobileSiderOpen ? 'center' : 'flex-start',
    },
    icon: {
  ...
  const renderTreeView = (tree: ITreeMenu[], selectedKey?: string) => {
    return tree.map((item, index) => {
      ...
      const additionalLinkProps = isParent ? {} : { component: Link as any, to: route };
  ...
  const dashboard = hasDashboard ? (
    <CanAccess resource="dashboard" action="list">
      <Tooltip label={t('dashboard.title', 'Dashboard')} {...commonTooltipProps}>
        <NavLink
          key="dashboard"
          label={siderCollapsed && !mobileSiderOpen ? null : t('dashboard.title', 'Dashboard')}
  ...
  const logout = isExistAuthentication && (
    <Tooltip label={t('buttons.logout', 'Logout')} {...commonTooltipProps}>
      <NavLink
        key="logout"
        label={siderCollapsed && !mobileSiderOpen ? null : t('buttons.logout', 'Logout')}
